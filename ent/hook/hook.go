...
// The IdPConnectorFunc type is an adapter to allow the use of ordinary
// function as IdPConnector mutator.
type IdPConnectorFunc func(context.Context, *ent.IdPConnectorMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f IdPConnectorFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.IdPConnectorMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.IdPConnectorMutation", m)
}
...
func (f OAuth2ClientFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.OAuth2ClientMutation); ok {
		return f(ctx, mv)
	}
